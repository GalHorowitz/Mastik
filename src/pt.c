...
pt_t pt_prepare(int associativity, vlist_t evset) {
  pt_t p = (pt_t) malloc(sizeof(struct prime_time));
  p->patterns = (ppattern_t*) malloc(sizeof(ppattern_t) * PATTERN_LIST_CAPACITY);
  p->pattern_count = 0;
...
void test_pattern(pt_t patterns, ppattern_t pattern, int test_count, float* evcr_result) {
  ...
  int associativity = patterns->associativity;
  ...
  for(int t = 0; t < test_count; t++) {
    ...
    uint32_t t_l1 = 0;
    for(int i = 0; i < 100; i++) {
      uint32_t t = memaccesstime(vl_get(evset, 0));
      if(t > t_l1) t_l1 = t;
    }
...
void time_pattern(pt_t patterns, ppattern_t pattern, int test_count, int* time_result) {
  ...
  for(int t = 0; t < test_count; t++) {
    access_pattern(patterns->evset, patterns->associativity, pattern);
  }
...
void mutate_repeat_subpatterns(pt_t patterns, bool less) {
  ...
  for(int i = 0; i < cur_count; i++) {
    ...
    if(pat.length + sub_len > PATTERN_CAPACITY) {
      printf("%d + %d = %d > %d\n", pat.length, sub_len, pat.length + sub_len, PATTERN_CAPACITY);
    }
...
pt_results_t generate_prime_patterns(pt_t patterns) {
  ...
  // 5. Measurements: Test 1'000'000 times
  //  - Filter to 150 Highest EVCr
  //  - Filter to 100 fastest  
  filter_evcr(patterns, 1000000, 150);
